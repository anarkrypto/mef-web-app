  ...
  try {
    ...
    const proposalsWithSummaries = await Promise.all(
      proposals.map(async (proposal: DeliberationProposal) => {
        const gptSummary = await gptSurveyService.getProposalSummary(proposal.id);
        return {
          ...proposal,
          gptSurveySummary: gptSummary && gptSummary.summary && gptSummary.summary_updated_at ? {
    ...
    const pendingCount = proposalsWithSummaries.reduce((count: number, p: DeliberationProposal) => 
      !p.userDeliberation ? count + 1 : count, 0
    );
