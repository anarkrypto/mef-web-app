	...
	try {
		...
		if (roundId) {
			...
			results = proposals.map(proposal => ({
				...
				summaryUpdatedAt:
					proposal.GptSurveySummarizerProposal?.summary_updated_at || undefined,
				feedbacks:
					proposal.deliberationCommunityVotes?.map(vote => ({
						voteId: vote.id,
						username:
							(vote.user?.metadata as UserMetadata | undefined)?.username ||
							'anonymous',
