...
function isUserMetadata(value: unknown): value is UserMetadata {
	...
	if (!metadata.authSource || typeof metadata.authSource !== 'object')
		return false
...
export class AdminFeedbackService {
		...
		try {
			...
			const typedItems = items.map(item => {
				if (!isUserMetadata(item.user.metadata)) {
					...
					throw new Error(
						`Invalid user metadata format for user ${item.user.id}`,
					)
				}
				return {
					...item,
	...
	async getFeedbackById(id: string): Promise<
		| (UserFeedback & {
				user: {
					id: string
					metadata: UserMetadata
	...
	> {
		try {
			...
			if (!isUserMetadata(feedback.user.metadata)) {
				logger.error(
					`Invalid user metadata format for user ${feedback.user.id}`,
				)
				throw new Error(
					`Invalid user metadata format for user ${feedback.user.id}`,
				)
