...
function parseMinReviewerApprovals(): number {
	...
	if (minApprovals !== undefined) {
		...
		if (isNaN(parsed) || parsed < 1) {
			logger.warn(
				`Invalid CONSIDERATION_REVIEWER_APPROVAL_THRESHOLD value: ${minApprovals}, using default: ${DEFAULT_CONSIDERATION_REVIEWER_APPROVAL_THRESHOLD}`,
			)
...
export class ProposalStatusMoveService {
	...
	async checkAndMoveProposal(proposalId: number): Promise<MoveResult | null> {
		...
		if (
			![
				ProposalStatus.CONSIDERATION.toString(),
				ProposalStatus.DELIBERATION.toString(),
			].includes(proposal.status.toString())
		) {
			return null
		}

		const shouldMove =
			proposal.status === ProposalStatus.CONSIDERATION
		...
		if (shouldMove) {
			...
			const ocvData = proposal.OCVConsiderationVote?.voteData as
				| OCVVoteResponse
				| undefined
			const isElegible = ocvData?.elegible ?? false

			const thresholdReviewerApprovals =
				this.config.considerationPhase.minReviewerApprovals
	...
	private async shouldMoveToDeliberation(
		proposal: ProposalWithVotes,
	): Promise<boolean> {
		const approvalCount = this.countValidApprovals(proposal)
		const isMinApprovals =
			approvalCount >= this.config.considerationPhase.minReviewerApprovals
		const ocvData = proposal.OCVConsiderationVote?.voteData as
			| OCVVoteResponse
			| undefined
		...
		logger.debug(
			`Proposal ${proposal.id} should move to DELIBERATION. Approval count: ${approvalCount}, min approvals: ${this.config.considerationPhase.minReviewerApprovals}, OCV elegible: ${ocvElegible}`,
		)

		return isMinApprovals || ocvElegible
	}

	private async shouldMoveBackToConsideration(
		proposal: ProposalWithVotes,
	): Promise<boolean> {
		const approvalCount = this.countValidApprovals(proposal)
		const isMinApprovals =
		...
		logger.debug(
			`Proposal ${proposal.id} should move back to CONSIDERATION. Approval count: ${approvalCount}, min approvals: ${this.config.considerationPhase.minReviewerApprovals}, OCV elegible: ${ocvElegible}`,
		)
	...
	private countValidApprovals(proposal: ProposalWithVotes): number {
		...
		return proposal.considerationVotes.filter(
			vote =>
