...
export function DeliberationDialog({
	...
	const getChangeSummary = () => {
		...
		if (isReviewer && currentRecommendation !== existingVote.recommendation) {
			changes.push(
				`Changed recommendation to ${currentRecommendation ? 'Recommend' : 'Not Recommend'}`,
			)
		}
						...
						{isReviewer && (
							<div className="flex gap-2">
								...
								<Button
									type="button"
									variant={
										currentRecommendation === false ? 'destructive' : 'outline'
									}
									className="flex-1"
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								isLoading ||
								!feedback.trim() ||
								(isReviewer && currentRecommendation === undefined)
							}
							variant={
						...
						<AlertDialogDescription asChild>
							<div className="space-y-2">
								<div>
									...
									{isReviewer ? 'review' : 'deliberation'}?
								</div>
