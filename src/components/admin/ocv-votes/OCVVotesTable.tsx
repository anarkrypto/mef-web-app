...
type SortField = 'proposalId' | 'ocvStatus' | 'reviewerProgress' | 'communityVotes' | 'fundingRound' | 'status';
  ...
  const sortedVotes = useMemo(() => {
    ...
    sortedData.sort((a, b) => {
      switch (sortConfig.field) {
        ...
        case 'communityVotes':
          return sortConfig.order === 'asc' ? 
            a.voteData.total_positive_community_votes - b.voteData.total_positive_community_votes :
            b.voteData.total_positive_community_votes - a.voteData.total_positive_community_votes;
        case 'fundingRound':
          return sortConfig.order === 'asc' ? 
            a.proposal.fundingRoundName.localeCompare(b.proposal.fundingRoundName) :
            b.proposal.fundingRoundName.localeCompare(a.proposal.fundingRoundName);
        case 'status':
          return sortConfig.order === 'asc' ? 
              ...
              <TableRow key={vote.id}>
                <TableCell>
                  <Button
                    ...
                    onClick={() => window.location.href = `/proposals/${vote.proposalId}`}
                  >
                    {vote.proposalId}
                  </Button>
                </TableCell>
                ...
                <TableCell>
                  <Badge 
                    variant={
                      vote.proposal.status === 'CONSIDERATION' ? 'default' :
                      vote.proposal.status === 'DELIBERATION' ? 'secondary' :
                      'outline'
                    }
                  >
                    {vote.proposal.status.charAt(0) + vote.proposal.status.slice(1).toLowerCase()}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Badge variant={vote.voteData.elegible ? "outline" : "secondary"}>
                      ...
                      <div>
                        ...
                        <div className="flex gap-3 text-xs">
                          ...
                          <span className="text-red-600">
                            Against: {vote.voteData.total_negative_community_votes}
                          </span>
                        </div>
                      </div>
                      <HoverCard>
                        ...
                        <HoverCardContent className="w-80">
                          <div className="space-y-2">
                            <pre className="rounded bg-muted p-2 text-sm overflow-auto max-h-[400px]">
