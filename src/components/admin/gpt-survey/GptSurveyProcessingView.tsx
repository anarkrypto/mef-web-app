...
export function GptSurveyProcessingView({ fundingRounds }: Props) {
  ...
  useEffect(() => {
    const loadRoundData = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`/api/admin/gpt-survey/status?roundId=${selectedRoundId}`);
        if (!response.ok) {
        ...
        toast({
          title: "Error",
          description: error instanceof Error ? error.message : "Failed to load funding round data",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
    ...
    pollInterval.current = setInterval(async () => {
      try {
        ...
        if (!data.isRunning && selectedRoundId) {
          stopPolling();
          const resultsResponse = await fetch(`/api/admin/gpt-survey/status?roundId=${selectedRoundId}`);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "GPT Survey processing has been initiated. Please wait for it to complete.",
      });
    } catch (error) {
      stopPolling();
      toast({
  ...
  const handleForceKill = async () => {
    try {
      ...
      toast({
        title: "Success",
        description: "Running job has been killed. You can now start a new process.",
      });
      stopPolling();
    } catch (error) {
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to kill running job",
        variant: "destructive",
      });
    }
  };
      ...
      <AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will forcefully terminate the running GPT Survey processing job. 
              This action cannot be undone. Check{" "}
              <Link href="/admin/worker-heartbeats" className="text-primary hover:underline">
                Worker Heartbeats
              </Link>
  ...
  return (
    <div className="space-y-6">
      ...
      {lastExecution && (
        <Alert variant={lastExecution.status === WorkerStatus.COMPLETED ? "default" : "destructive"}>
          <ClockIcon className="h-4 w-4" />
          <AlertDescription className="ml-2">
            Last execution: {formatDate(lastExecution.timestamp)} - Status: {lastExecution.status}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                <p className="text-sm text-muted-foreground">
                  When checked, summaries will be generated for all proposals, even if no new feedback was submitted.
                </p>
              </div>
            </div>
          </div>
