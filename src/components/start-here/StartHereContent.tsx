...
export function StartHereContent() {
	...
	useEffect(() => {
		async function fetchActiveFundingRounds() {
			try {
				...
				if (!response.ok)
					throw new Error('Failed to fetch active funding rounds')
		...
		return (
			<Card className="mx-auto mt-12 max-w-2xl">
				<CardHeader>
					...
					<CardDescription>
						There are currently no active funding rounds. Please check back
						later.
					</CardDescription>
				</CardHeader>
			</Card>
	...
	const selectedRoundData = activeFundingRounds.find(
	...
	return (
		<div className="space-y-12">
						...
						<Select value={selectedRound} onValueChange={setSelectedRound}>
							...
							<SelectContent>
								{activeFundingRounds.map(round => (
									<SelectItem key={round.id} value={round.id} className="py-3">
					...
					<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
						...
						>
							<CardHeader>
								...
								<CardDescription>
									View and review submitted proposals in the current funding
									round
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<Card className="transition-all duration-200 hover:shadow-md">
							...
							<CardContent>
								<Button
									variant="outline"
									onClick={() =>
										router.push(`/funding-rounds/${selectedRound}/summaries`)
									}
									className="w-full"
								>
			...
			<div className="mt-16">
				...
				<FAQ sections={faqData} selectedFundingRound={selectedRoundData} />
			</div>
		</div>
	)
}
