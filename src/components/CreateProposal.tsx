...
const proposalSchema = z.object({
	...
	deliveryRequirements: z
		.string()
		.min(
			PV.DELIVERY_REQUIREMENTS.MIN,
			PV.DELIVERY_REQUIREMENTS.ERROR_MESSAGES.MIN,
		)
		.max(
			PV.DELIVERY_REQUIREMENTS.MAX,
			PV.DELIVERY_REQUIREMENTS.ERROR_MESSAGES.MAX,
		),

	securityAndPerformance: z
		.string()
		.min(
			PV.SECURITY_AND_PERFORMANCE.MIN,
			PV.SECURITY_AND_PERFORMANCE.ERROR_MESSAGES.MIN,
		)
		.max(
			PV.SECURITY_AND_PERFORMANCE.MAX,
			PV.SECURITY_AND_PERFORMANCE.ERROR_MESSAGES.MAX,
		),
...
export default function CreateProposal({ mode = 'create', proposalId }: Props) {
	...
	const { handleSubmit, loading } = useFormFeedback({
		...
		errorMessage:
			mode === 'edit' ? 'Failed to update proposal' : 'Failed to save proposal',
	...
	const onSubmit = async (e: React.FormEvent) => {
		...
		await handleSubmit(async () => {
			...
			const url =
				mode === 'edit' ? `/api/proposals/${proposalId}` : '/api/proposals'
				...
				<form onSubmit={onSubmit} className="space-y-8">
					<div className="space-y-4">
						...
						<p className="text-sm text-muted-foreground">
							{getRemainingChars('proposalName', PV.NAME.MAX)} characters
							remaining
						</p>
					</div>
					...
					<div className="space-y-4">
						...
						<p className="text-sm text-muted-foreground">
							{getRemainingChars('abstract', PV.ABSTRACT.MAX)} characters
							remaining
						</p>
					</div>
					...
					<div className="space-y-4">
						...
						<p className="text-sm text-muted-foreground">
							{getRemainingChars('motivation', PV.MOTIVATION.MAX)} characters
							remaining
						</p>
					</div>
					...
					<div className="space-y-4">
						...
						<p className="text-sm text-muted-foreground">
							{getRemainingChars('rationale', PV.RATIONALE.MAX)} characters
							remaining
						</p>
					</div>
					...
					<div className="space-y-4">
						<Label
							htmlFor="securityAndPerformance"
							className="text-xl font-semibold"
						>
