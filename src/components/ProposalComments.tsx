...
export function ProposalComments({ comments }: ProposalCommentsProps) {
	...
	const sortedComments = useMemo(() => {
		...
		const allComments = [
			...(comments.reviewerConsideration || []).map(c => ({
				...c,
				category: 'reviewerConsideration' as const,
			})),
			...(comments.reviewerDeliberation || []).map(c => ({
				...c,
				category: 'reviewerDeliberation' as const,
			})),
			...(comments.communityDeliberation || []).map(c => ({
				...c,
				category: 'communityDeliberation' as const,
			})),
		]
						...
						<AccordionContent>
							<div className="space-y-4 rounded-lg bg-muted/50 p-4">
								<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
									...
									Last updated:{' '}
									{new Date(
										comments.gptSurveySummary.summaryUpdatedAt,
									).toLocaleString()}
								</div>
								<div className="prose prose-sm dark:prose-invert max-w-none">
									<ReactMarkdown>
										{comments.gptSurveySummary.summary}
									</ReactMarkdown>
								</div>
							</div>
						</AccordionContent>
...
function truncateUsername(
	username: string,
	maxLength: number = 20,
): { truncated: string; isShortened: boolean } {
			...
			className={cn(
				...
				comment.recommendation !== undefined
					? comment.recommendation
						? 'border-green-500/20 bg-green-50/50 hover:border-green-500/30 dark:bg-green-900/10'
						: 'border-red-500/20 bg-red-50/50 hover:border-red-500/30 dark:bg-red-900/10'
					: 'border-gray-200 bg-gray-50/50 hover:border-gray-300 dark:bg-gray-900/10',
			)}
		>
								...
								{comment.isReviewerComment ? (
									<div className="space-y-1">
										...
										<p className="text-sm text-muted-foreground">
											This comment is from a verified expert reviewer for this
											proposal.
										</p>
									</div>
								) : (
									<div className="space-y-1">
										...
										<p className="text-sm text-muted-foreground">
											Community members&lsquo; identities are kept anonymous to
											ensure unbiased deliberation.
										</p>
									</div>
								)}
							</TooltipContent>
