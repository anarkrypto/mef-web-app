...
export function FundingRoundStatus({ onRoundSelect }: Props) {
	const getCurrentPhase = (round: FundingRound): PhaseType | null => {
		...
		if (
			now >= new Date(round.submissionPhase.startDate) &&
			now <= new Date(round.submissionPhase.endDate)
		) {
			return 'submission'
		}
		if (
			now >= new Date(round.considerationPhase.startDate) &&
			now <= new Date(round.considerationPhase.endDate)
		) {
			return 'consider'
		}
		if (
			now >= new Date(round.deliberationPhase.startDate) &&
			now <= new Date(round.deliberationPhase.endDate)
		) {
			return 'deliberate'
		}
		if (
			now >= new Date(round.votingPhase.startDate) &&
	...
	const currentPhase = selectedRound
		? getCurrentPhase(selectedRound)
		: ('submission' as PhaseType)
		...
		const fetchFundingRounds = async () => {
			try {
				...
				const activeRound = rounds.find(
					(round: FundingRound) => round.status === 'ACTIVE',
				)
				if (activeRound) {
								...
								<TooltipTrigger asChild>
									<div>
										<Button
											...
											onClick={() =>
												!isUpcoming(round) && setSelectedRoundId(round.id)
											}
											className={cn(
												'min-w-[120px]',
												isUpcoming(round) && 'cursor-not-allowed opacity-50',
				...
				{selectedRound && (
					<>
								...
								<div className="grid grid-cols-4 gap-4 text-center">
									<div>
										<div className="text-4xl font-bold">
											📝 {selectedRound.proposals?.length || 0}
										</div>
										...
										<div className="text-4xl font-bold">
											{
												getTimeRemainingWithEmoji(
													new Date(selectedRound.endDate),
												).emoji
											}{' '}
											{
												getTimeRemainingWithEmoji(
												...
												getTimeRemainingWithEmoji(
													new Date(
														currentPhase === 'consider'
															? selectedRound.considerationPhase.endDate
															: currentPhase === 'deliberate'
																? selectedRound.deliberationPhase.endDate
						...
						<div className="grid grid-cols-[200px,1fr] gap-8">
							{/* Phase Progress */}
							<div className="space-y-4">{renderPhaseTimeline()}</div>
								...
								{currentPhase === 'submission' && (
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Submit your proposals for this funding round. Review
												other submissions and provide feedback.
											</CardDescription>
										</CardHeader>
									</Card>
								)}
								...
								{currentPhase === 'consider' && (
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Review submitted proposals and determine which ones you
												find valuable enough to receive funding.
											</CardDescription>
										</CardHeader>
									</Card>
								)}
								...
								{currentPhase === 'deliberate' && (
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Discuss and refine proposals with the community before
												final voting.
											</CardDescription>
										</CardHeader>
									</Card>
								...
								{currentPhase === 'vote' && (
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Cast your votes to determine which proposals will
												receive funding.
											</CardDescription>
										</CardHeader>
									</Card>
