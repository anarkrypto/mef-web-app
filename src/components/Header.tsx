...
import { Bell, Settings, ChevronDown, ArchiveIcon, ActivityIcon } from 'lucide-react'
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-14 items-center mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-1 items-center justify-between md:justify-start">
          ...
          <nav className="hidden md:ml-12 md:flex items-center space-x-8 text-sm font-medium">
            {navigation.map((item) => (
              <Link
                key={item.href}
                href={item.href}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button 
                  ...
                  className={cn(
                    "flex items-center space-x-2 transition-colors hover:text-foreground/80",
                    pathname.includes('/funding-rounds') && pathname.includes('/summaries')
                      ? "text-foreground font-semibold"
                      : "text-foreground/60"
                  )}
                >
