    ...
    return (
      <div className="rounded-lg border bg-white/95 p-3 shadow-lg backdrop-blur-sm">
        ...
        <p className="text-xs text-gray-600">
          Count: <span className="font-medium">{data.value}</span> ({((data.value / data.total) * 100).toFixed(1)}%)
        </p>
      </div>
    );
  }
...
export const BudgetDistributionChart: FC<BudgetDistributionChartProps> = ({
  ...
  const totalProposals = Object.values(budgetBreakdown).reduce((a, b) => a + b, 0);
  
  const pieData = [
    { name: '100-500 MINA', value: budgetBreakdown.small, total: totalProposals },
    { name: '500-1000 MINA', value: budgetBreakdown.medium, total: totalProposals },
    ...
    <Card className="h-[420px]">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-2">
          <p className="text-sm text-muted-foreground">
            Total: <span className="font-medium text-foreground">{totalProposals}</span>
          </p>
