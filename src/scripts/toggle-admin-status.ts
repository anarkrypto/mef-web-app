...
async function setAdminStatus(userId: string, status: boolean): Promise<void> {
	if (status) {
		...
		console.log(
			`${colors.green}✓${colors.reset} Admin status ${colors.bright}enabled${colors.reset}`,
		)
	} else {
		...
		console.log(
			`${colors.yellow}✓${colors.reset} Admin status ${colors.bright}disabled${colors.reset}`,
		)
	}
}
...
async function main() {
	try {
		...
		if (!userId) {
			userId = await question(
				`${colors.yellow}?${colors.reset} Enter user ID: `,
			)
		}
		...
		if (action !== 'exit') {
			...
			console.log(
				`\n${colors.bright}New Status:${colors.reset} ${
					newStatus
						? `${colors.green}✓ Enabled${colors.reset}`
						: `${colors.red}✗ Disabled${colors.reset}`
				}`,
